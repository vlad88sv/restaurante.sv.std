...
$c = 'SELECT (STDDEV(TIME_TO_SEC(TIMEDIFF(`fechahora_entregado`, `fechahora_pedido`))) / 60) AS stddev_tps, (AVG(TIME_TO_SEC(TIMEDIFF(`fechahora_entregado`, `fechahora_pedido`))) / 60) AS tps FROM `ordenes` WHERE  `fechahora_pedido` BETWEEN "'.$periodo_inicio.'" AND "'.$periodo_final.'" AND flag_elaborado=1 AND fechahora_entregado <> "0000-00-00 00:00:00"';
$r = db_consultar($c);
$f = db_fetch($r);
