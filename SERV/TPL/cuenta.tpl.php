...
$c = 'SELECT (SELECT nota FROM historial st0 WHERE grupo="PEDIDOS" AND st0.ID_pedido = t1.ID_pedido ORDER BY ID_historial DESC LIMIT 1) AS "historia", t0.ID_mesero, t4.usuario AS "nombre_mesero", t0.cuenta, t0.`fechahora_pedido` , unix_timestamp(t0.`fechahora_pedido`) AS "fechahora_pedido_uts" , t0.`fechahora_entregado` , t0.`fechahora_pagado` , t0.`flag_nopropina` , t0.`flag_exento` , t0.`flag_pagado` , t0.`flag_elaborado` ,  t0.`flag_anulado`, t0.`flag_tiquetado`, t1.`flag_cancelado`, t0.`metodo_pago` , t1.`ID_orden` , t0.`ID_mesa` , t0.`ID_usuario` , t1.`ID_pedido` , `ID_producto` , `precio_grabado` , t2.`nombre` AS "nombre_producto", `tmpID`, `flag_cancelado`, t2.ID_grupo, t3.descripcion AS "grupo_desc"
FROM `ordenes` AS t0
