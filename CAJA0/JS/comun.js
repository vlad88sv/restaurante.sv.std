...
function agregarPedido(objetivo, grupo, modo)
{
    // Modo = 0 : normal
    // Modo = 1 : historial
    ...
    for (x in _ordenes[grupo])
    {
        ...
        buffer += '<span class="estado_despacho" title="P = pendiente | D = despachado">' + (_ordenes[grupo][x].flag_elaborado === '0' ? 'P' : 'D') + '</span>&nbsp;';
        buffer += botones;
